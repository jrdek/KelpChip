	...
	for (int byte_num = 0; byte_num < n; byte_num++) {
		...
		for (int bit = 0; bit < 8; bit++) {
			...
			for (int ctr = 0; ctr < 4; ctr++) {
				gfx_colors[ctr + 4*((64*(y%32))+((x+bit)%64))] = gfx[(64*(y%32)) + ((x+bit)%64)] ? 0xff : 0x00;  // rgba
			}
			if (V[0xF] == 0) {
...
void chip8::exec_next() {
		...
		case 0: 
			if (opcode == 0x00e0) {
				...
				*SP = *SP - 1;
				...
				// sys; not supported. Treat it as a NOP!
	...
	*PC += 2;
	if (limiter_clock.getElapsedTime().asMilliseconds() > 16) {
		if (*delay_timer) {
			*delay_timer -= 1;
		}
		if (*sound_timer) {
